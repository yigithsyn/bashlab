...
int main(int argc, char *argv[])
{
  ...
  if (help->count > 0)
  {
    ...
    printf("%s <wavelength> [-h|--human] [-o|--out=<outfile>] \n", PROGNAME);
  ...
  )
  {
    /* Display the error details contained in the arg_end struct.*/
    arg_print_errors(stdout, end, PROGNAME);
    printf("One of <wavelength>, <filename> or pipe should be supplied only.\n");
    printf("Try '%s --help' for more information.\n", PROGNAME);
    exitcode = EXIT_FAILURE;
    goto EXIT;
  }
...
OUTPUT:
  for (int i = 0; i < N; ++i)
    (human->count == 1) ? fprintf(fout, "%s\n", wavelen2freq_h(freqs[i], buff))
                        : fprintf(fout, "%f\n", wavelen2freq(freqs[i]));
