...
static const char *program_json =
    "{"
    "\"name\": \"wavelen2freq\","
    "\"desc\": \"convert wavelength to frequency\","
    "\"pargs\": ["
    /*        */ "{\"name\":\"wavelength\", \"minc\":1, \"maxc\":100, \"desc\":\"wavelength value(s)\"}"
    /*       */ "],"
    "\"oargs\": ["
    /*        */ ""
    /*       */ "],"
    "\"opts\": ["
    /*       */ "{\"short\":\"h\", \"long\":\"human\", \"desc\":\"human readable display like 100 MHz, 2.3 GHz\"}"
    /*      */ "]"
    "}";
...
int main(int argc, char *argv[])
{
  ...
  /* main operation                                                           */
  /* ======================================================================== */
  struct arg_str *arg1 = (struct arg_str *)argtable[0]; // wavelengths
...
{
  if (arg >= 1E12)
    sprintf(buff, "%.1f THz", arg / 1E12);
  else if (arg >= 1E9)
    sprintf(buff, "%.1f GHz", arg / 1E9);
  else if (arg >= 1E6)
    sprintf(buff, "%.1f MHz", arg / 1E6);
  else if (arg >= 1E3)
    sprintf(buff, "%.1f kHz", arg / 1E3);
  else
    sprintf(buff, "%.1f Hz", arg / 1E0);
  return buff;
}
